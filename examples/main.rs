...
pub fn main() {
    ...
    // verify vote
    println!("vote is verified: {}", context.verify(&vote)); // true
    ...
    println!(
        "vote after deserialization is verified: {}", // true
        context.verify(&vote)
    );

    // issue new vote for choice 1 with the same private key
    let vote_2 = context.issue(private_key.clone(), 1, csprng);
    println!("vote2 is verified: {}", context.verify(&vote_2)); // true

    // link votes
    println!(
        "vote and vote2 is signed with same private key: {}", // true
    ...
    println!("vote3 is verified: {}", context.verify(&vote_3)); // true

    // link votes
    println!(
        "vote and vote3 is signed with same private key: {}", // false
        context.link(&vote, &vote_3)
    );
    println!(
        "vote2 and vote3 is signed with same private key: {}", // false
